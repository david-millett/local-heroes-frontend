...
[Local Heroes](https://localheroesapp.netlify.app/)

## Repositories

This project is comprised of two repositories, one for the front end and one for the back end.

To view the code for this project, please visit the appropriate GitHub repository:
...
This project was created as part of a team of three over the course of one week.
...
As a team, we divided tasks and all contributed to the front-end and back-end portions of the project. My specific contributions are detailed in this Readme.
...
Working on my own individual branch `/david/helpers-backend`, I created my parts of the back end, including the initial helpers model and controller file, which implemented full CRUD, as we required create, index, update, and delete routes for the helper profiles.

At each stage, I thoroughly tested the routes one-by-one in *Postman* to ensure that everything behaved as intended. Error handling was included.

I had to be mindful that certain routes - specifically the update and delete routes - should be protected by authorisation. As my teammate Iris was developing the user model and routes concurrently with me developing this, I commented in where code referencing the user would go after merging with the group.

Following our first merge, we extensively tested all of the routes and added in things like authorisation that could now be properly implemented.
...
Because there were some similarities across similar pages - for example, we had two versions of show pages, it made sense at this stage to divide tasks differently. I designed the show pages for both jobs and helpers, ensuring that they dynamically showed the relevant data like profile images and dates for each user.
...
Sometimes, what initially seemed to be a problem was an easy solve. At one stage I had issues the show pages breaking when refreshing - they were losing access to the helper's information that was required to display the page. From the error log in the console, I could see this was an authentication error - this made me eventually realise that the issues were simply because the token had expired, and not an issue with the page itself. I had stayed logged in for longer than normal because our sign out had stopped working after one of our merges, and my team mate was looking into it. In dev tools, I headed to the application tab and deleted the token to force a log out. Once I freshly logged in, the error was no longer there. This taught me to check this early on in problem solving in future projects, as it could be an easy explanation for some errors.
...
As can been, this delete button is also only rendered when the user who made the comment/testimonial is logged in. This then allowed me to make the `handleDeleteTestimonial()` and `handleDeleteComment()` functions use the event target (`e.target.id`) to specify which testimonial/commment to delete.
...
We went into this project with an ambitious plan, and sadly we did not get the chance to include a lot of our ideas. This is partially because it took us a lot longer to sort through undoreseen hurdles that came about as a result of working as a group.

I think some obvious improvements would be to incorporate some of these features. Two of my favourite features we didn't get to include were the badges system and local feed. I think that both of these feautures would have helped to build the community feel of the project, which is what we were trying to create.
...
Speaking of which, incorporating location better - like we'd originally planned - would definitely help the website. We decided at first to centre it on London, but it would be good to roll out nationally and have users submit more robust location data. That way, we could filter jobs and helpers to ensure only those nearby to the user are shown.
